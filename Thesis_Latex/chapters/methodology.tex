...
\begin{figure}[htbp]
    ...
    \node[newprocbox] (ctxextract) at (2.75, -2) {\textbf{Automated Context}\\
    \textbf{Extraction}\\{\footnotesize(cifuzz spark \cite{cifuzzspark})}};

    \node[newdatabox] (apictx) at (2.75, -4.2) {Extracted API Context\\{\footnotesize(signatures, types, classes)}};

    % ============ GENERATION LAYER ============
    \node[newdatabox] (fuzzinstr) at (-2.5, -6.4) {Fuzzing Instructions\\{\footnotesize(libFuzzer harness rules,}\\{\footnotesize safety constraints)}};

    \node[newprocbox] (promptbuild) at (2.75, -6.8) {\textbf{Prompt Construction}\\{\footnotesize(API context + instructions)}};

    \node[newdatabox] (prompt) at (2.75, -9) {Assembled Prompt};

    \node[databox] (llmmodel) at (8, -9) {LLM Model\\{\footnotesize(local via Ollama /}\\{\footnotesize cloud via Azure OpenAI)}};

    \node[newprocbox] (llmgen) at (5.5, -11) {\textbf{LLM-Based Driver}\\
    \textbf{Generation}};

    \node[newdatabox] (gendriver) at (5.5, -13) {Generated Fuzz Driver\\{\footnotesize(C++ source code)}};

    % ============ EXECUTION LAYER ============
    \node[procbox] (compile) at (2.5, -15) {\textbf{Compilation}\\{\footnotesize(Clang + ASan/UBSan)}};

    \node[databox] (binary) at (2.5, -17) {Instrumented Binary};

    \node[databox] (seedinputs) at (8.5, -17) {Seed Inputs};

    \node[procbox] (fuzzexec) at (5.5, -19) {\textbf{Fuzzing Execution}\\{\footnotesize(libFuzzer)}};

    \node[databox] (covdata) at (8.5, -21) {Coverage Data};
    \node[databox] (crashdata) at (2.5, -21) {Crash Reports};

    \node[newprocbox] (coveval) at (8.5, -23) {\textbf{Coverage Evaluation}\\{\footnotesize(quality gate)}};

    \node[newdatabox] (covreport) at (8.5, -25) {Coverage Report\\{\footnotesize(pass / fail)}};
