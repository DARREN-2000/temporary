...
\subsection{CI/CD/CT Pipeline Context}
...
\begin{figure}[htbp]
    ...
    \adjustbox{max width=0.95\textwidth, max height=0.65\textheight}{%
    \begin{tikzpicture}[
        databox/.style={draw, rectangle, minimum width=3.2cm, minimum height=0.9cm, align=center, fill=blue!8, thick, font=\small},
        procbox/.style={draw, rounded corners=8pt, rectangle, minimum width=3.2cm, minimum height=0.9cm, align=center, fill=orange!12, thick, font=\small},
        arrow/.style={->, >=stealth, thick},
        every node/.style={inner sep=5pt}
    ]
    % Zigzag layout: process+data pairs side by side, alternating left/right
    % Row 0: Commit (center top)
    \node[databox] (commit) at (3.5, 0) {Commit /\\Source Tree};

    % Row 1: Build (left) → Artifact (right)
    \node[procbox] (build) at (0, -2) {Build\\{\footnotesize(CMake / Clang)}};
    \node[databox] (artifact) at (7, -2) {Build Artifact\\{\footnotesize(Binary / Library)}};

    % Row 2: Test (right) → Results (left)
    \node[procbox] (test) at (7, -4) {Test Execution\\{\footnotesize(Unit / Integration)}};
    \node[databox] (testresults) at (0, -4) {Test Results};

    % Row 3: Security Validation (left, highlighted) → Report (right)
    \node[procbox, fill=yellow!20] (secval) at (0, -6) {\textbf{Security Validation}\\{\footnotesize(Fuzzing / SAST / DAST)}};
    \node[databox] (secreport) at (7, -6) {Security Report /\\Findings};

    % Row 4: Deploy (right) → Release (left)
    \node[procbox] (deploy) at (7, -8) {Deploy\\{\footnotesize(Staging / Production)}};
    \node[databox] (release) at (0, -8) {Deployed Release};

    % Row 5: Monitor (left) → Telemetry (right)
    \node[procbox] (monitor) at (0, -10) {Monitor\\{\footnotesize(Telemetry / Logging)}};
    \node[databox] (telemetry) at (7, -10) {Telemetry /\\Incident Data};

    % Arrows following the zigzag flow
    ...
    % Feedback loop
    \draw[arrow, dashed] (telemetry.south) -- ++(0,-0.6) -| node[font=\footnotesize, pos=0.25, below]{Feedback} (commit.south);
...
Everyone in machine learning hears that bigger models are better. Models with more parameters consistently outperform smaller ones on standard benchmarks \cite{Jiang2023}. The largest models show impressive capabilities across diverse tasks. They are also expensive (cloud API costs of \$0.01--\$0.06 per 1k tokens, or dedicated GPU hardware costing \$10,000+ per unit), and often have rate limits. But domain-specific tuning offers an alternative path \cite{Hu2021}. Instead of relying on the general knowledge of massive models, we adapt smaller models to specific tasks.

We wanted to know: which approach wins for fuzz driver generation?

\textbf{RQ3 (Feasibility): Can LLM-assisted fuzz driver generation be integrated into secure enterprise CI/CD/CT pipelines that are isolated from external networks while meeting performance, cost, and security requirements?}

Academic research often evaluates techniques in isolation, abstracting away deployment concerns. Production matters. Can the solution run within CI/CD/CT time constraints? Does the cost fit budget limits? Can it operate under network security policies that restrict external communication? Is operational complexity acceptable?
...
\textbf{Chapter 2: Literature Review} surveys relevant research. It examines traditional AI approaches in software testing, traces the evolution of Large Language Models and their application to code generation, reviews fuzzing techniques from foundational methods through AI-enhanced approaches, and examines prior work on security integration in CI/CD/CT pipelines.

\textbf{Chapter 3: Methodology and Framework} presents our approach. It introduces the conceptual framework for AI-driven fuzzing using white-box techniques, describes our technical architecture design, and outlines the three research phases: local LLM evaluation, model optimization with LoRA, and enterprise CI/CD/CT integration. We specify evaluation methodology and metrics.
