...
\caption{The V-Model Development Lifecycle (adapted from ISO~26262~\cite{ISO26262}). The diagram illustrates the relationship between each development phase (left) and its corresponding testing phase (right), converging at the Code implementation. Dashed arrows indicate requirement-to-test traceability.}
\label{fig:v_model}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{The Fuzzing Process. The manual creation of "Fuzz Drivers" (center) is the bottleneck this thesis aims to automate.}
    \label{fig:fuzzing_concept}
\end{figure}

The fuzzing architecture begins with the human element. The developer writes the initial source code and possesses the critical domain knowledge required to understand the intended software logic. Because an automated system cannot inherently know what specific areas to target, the developer must manually define the testing scope and generate the initial fuzz tests. This manual creation step requires significant effort and represents the primary bottleneck that this research aims to automate.

Once these initial tests are supplied, the automated fuzzing engine takes control. The engine continuously executes the target software using the provided inputs while actively monitoring the system for unexpected behavior. If a crash is detected, the engine extracts the crash data and generates a vulnerability report. Conversely, if the execution completes without error, the engine collects coverage data to understand which code paths were reached. It then mutates the previous inputs to create slightly altered variations. These mutated inputs are fed back into the execution loop, driving a continuous cycle of automated vulnerability discovery.
