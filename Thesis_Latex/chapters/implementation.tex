...
We turned the methodology from Chapter 3 into working code and systems over the course of this implementation. The three research phases defined in Chapter~3 (Section~\ref{sec:meth_strategy}), namely Phase~1: Local LLM Evaluation (Section~\ref{sec:impl_phase1}), Phase~2: Model Optimization with LoRA (Section~\ref{sec:impl_phase2}), and Phase~3: Enterprise CI/CD/CT Integration (Section~\ref{sec:impl_phase3}), map directly to the development progression described below. Each phase built on the previous one. The following sections cover our tool choices, the specific steps we followed, and the engineering decisions we made along the way.
...
export CIFUZZ_LLM_API_URL="https://<endpoint>.openai.azure.com"
...
Local development happened on a MacBook Pro with an M1 Pro chip. For Linux container support on Apple Silicon, we chose Podman as our container runtime. Podman aligned well with the enterprise CI/CD/CT runners, which also used Linux.
...
Installing these models required substantial disk space: the larger models exceeded 20~GB each in their quantized forms (see Table~\ref{tab:impl_model_summary} for per-model sizes). We used 4-bit and 5-bit quantization to fit models into available VRAM while maintaining reasonable output quality. A complete summary of all 14 evaluated models, including their parameter counts, quantization levels, architectures, and results, is provided in Table~\ref{tab:impl_model_summary} and in the appendix (Table~\ref{tab:appendix_all_models}).
...
\begin{figure}[htbp]
    ...
    % --- Row 2: cifuzz spark ---
    \node[comp] (cifuzz) at (6, -2.5) {\textbf{cifuzz spark}\\{\footnotesize Driver Generation}};

    % --- Row 3: Generated Driver ---
    \node[data] (driver) at (6, -5) {Generated Fuzz Driver\\{\footnotesize(C++ source code)}};

    % --- Row 4: Build ---
    \node[comp] (cmake) at (6, -7.5) {\textbf{CMake + Clang}\\{\footnotesize Build System}};

    % --- Row 5: Instrumented Binary ---
    \node[data] (binary) at (6, -10) {Instrumented Binary};

    % --- Row 6: Fuzzing Engine ---
    \node[comp] (libfuzzer) at (6, -12.5) {\textbf{libFuzzer}\\{\footnotesize Fuzzing Engine}};

    % --- Row 7: Coverage Measurement ---
    \node[comp] (llvmcov) at (0, -12.5) {\textbf{llvm-cov}\\{\footnotesize Coverage Tool}};
    \node[data] (coverage) at (0, -10) {Coverage Report};

    % --- Podman container runtime label ---
    \node[comp, fill=gray!8] (podman) at (12, -7.5) {\textbf{Podman}\\{\footnotesize Container Runtime}};
    ...
    % Container boundary
    \draw[dashed, thick, gray!60] (-2.5, -1.5) rectangle (14, -14);
    \node[font=\footnotesize\bfseries, gray] at (5.75, -14.4) {Podman Container Environment};
...
We used Low-Rank Adaptation (LoRA) \cite{Hu2021} rather than full fine-tuning. Full fine-tuning updates all model weights, which for a 7B-parameter model requires multiple high-end GPUs and days of training. LoRA adds small trainable matrices alongside the frozen base model. For our 1.5B model, this reduced the number of trainable parameters from 1.5 billion to approximately 4.7 million (a 99.7\% reduction), enabling training on a single consumer GPU while achieving similar results on specialized tasks.
...
We trained on the Qwen 2.5-Coder 1.5B base model. This is a small model that runs on a single consumer GPU with 8\,GB VRAM (e.g., Apple M1 Pro or NVIDIA RTX~3070). This size was chosen to investigate whether fine-tuning could make small models competitive with larger ones.
...
\section{Phase 3: Enterprise CI/CD/CT Integration}
\label{sec:impl_phase3}

Phase 3 focused on deploying the validated approach from Phases 1 and 2 into CARIAD's production CI/CD/CT infrastructure using Azure OpenAI \cite{azure_openai} as the LLM backend. The central question for this phase was \textbf{RQ3}: Can LLM-assisted fuzz driver generation be integrated into secure enterprise CI/CD/CT pipelines while meeting performance, cost, and security requirements? The transition from controlled local experiments with Ollama \cite{ollama} to enterprise deployment revealed additional infrastructure considerations that constituted the primary engineering focus of this phase.
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item Driver generation must complete within the CI/CD/CT feedback window (targeted at 15 to 30 minutes, with up to 45 minutes acceptable for complex libraries).
    ...
    \begin{enumerate}
        ...
        \item The solution must not require dedicated GPU hardware on the CI/CD/CT runners.
...
CARIAD operates with a zero-trust security model. CI/CD/CT runners are isolated from the public internet, and external connections require explicit approval. Integrating cloud-based AI services within this security framework required a tailored architectural approach.
...
\begin{lstlisting}
Error: fatal: unable to access 'https://<corporate-git>.example.com/...': 
...
\textbf{Attack model.} The container isolation addresses two distinct threat categories. First, \emph{tenant isolation}: in a shared runner environment, containers prevent one CI/CD/CT job from accessing another's data or network connections. Buildah's rootless, daemonless architecture strengthens this boundary compared to Docker, as there is no long-running daemon with elevated privileges that could be exploited. Second, \emph{generated code containment}: since LLM-generated fuzz drivers are untrusted code, they execute inside containers with restricted privileges, no shell access, and sanitizer instrumentation (ASan/UBSan) that terminates on memory violations.
...
Custom DNS entries:
\begin{lstlisting}[language=bash]
buildah run --add-host <endpoint>.openai.azure.com:${OPENAI_IP} ...
\end{lstlisting}
...
Once Private Link was operational, the pipeline worked reliably. Figure~\ref{fig:cicd_workflow} illustrates the CI/CD/CT pipeline as a workflow diagram. The full YAML configuration is provided in Appendix~\ref{sec:appendix_pipeline}.
...
\begin{figure}[htbp]
    ...
    % --- Row 1 ---
    \node[procstep] (checkout) at (0, -1.6) {Checkout\\Repository};
    \node[procstep] (login) at (5, -1.6) {Login to\\JFrog Artifactory};

    % --- Row 2 ---
    \node[procstep] (corpus) at (5, -3.2) {Download\\Previous Corpus};
    \node[procstep] (container) at (0, -3.2) {Pull cifuzz\\Container Image};

    % --- Row 3 ---
    \node[procstep] (build) at (2.5, -4.8) {Build Project\\{\footnotesize(CMake + Clang)}};

    % --- Row 4: LLM ---
    \node[procstep, fill=yellow!20] (spark) at (2.5, -6.4) {\textbf{cifuzz spark}\\{\footnotesize Generate Fuzz Driver}\\{\footnotesize via Azure OpenAI}};

    % --- Row 5 ---
    \node[procstep] (rebuild) at (2.5, -8) {Rebuild with\\Generated Driver};

    % --- Row 6 ---
    \node[procstep] (fuzz) at (2.5, -9.6) {Run libFuzzer\\{\footnotesize(60\,s, ASan+UBSan)}};

    % --- Row 7: outputs ---
    \node[data] (crashes) at (-1, -11.2) {Crash\\Reports};
    \node[data] (coverage) at (6, -11.2) {Coverage\\Data};

    % --- Row 8 ---
    \node[procstep] (upload) at (2.5, -12.8) {Upload Updated\\Corpus to Artifactory};
...
For context, even the enterprise scenario (\euro{}1,452 per year) is approximately two orders of magnitude below the cost of a single full-time security engineer (approximately \euro{}80,000 to \euro{}120,000 annually at industry rates). However, the LLM-generated drivers supplement rather than replace expert security engineering work, so this comparison captures only the infrastructure cost component. A detailed economic analysis including human oversight costs appears in Chapter~5.
...
Generated code running in CI/CD/CT pipelines poses security considerations. What if the LLM generates malicious code? We addressed this concern in our security review:
