...
Gemma 3 27B & Code-specialized & 27B & Q4\_K\_M & $\sim$17\,GB & 45.06\% & Functional \\
Phi-4 14B & Code-specialized & 14B & Q4\_K\_M & $\sim$9\,GB & 34.26\% & Functional \\
...
Gemma 3 9B & Code-specialized & 9B & Q4\_K\_M & $\sim$6\,GB & 0\% & Compile failure \\
Gemma 3 4B & Code-specialized & 4B & Q4\_K\_M & $\sim$3\,GB & 0\% & Compile failure \\
...
\begin{figure}[htbp]
    ...
    \resizebox{\textwidth}{!}{
    \begin{tikzpicture}[
        comp/.style={draw, rectangle, rounded corners=4pt, minimum width=3.2cm, minimum height=1cm, align=center, fill=blue!8, thick, font=\small},
        data/.style={draw, rectangle, minimum width=2.8cm, minimum height=0.8cm, align=center, fill=green!10, thick, font=\small},
    ...
    \node[comp] (llamacpp) at (0, -2.5) {\textbf{llama.cpp}\\{\footnotesize Alternative Backend}};
    \node[comp] (cifuzz) at (5.5, -1.25) {\textbf{cifuzz spark}\\{\footnotesize Driver Generation}};
    \node[comp] (cmake) at (11, 0) {\textbf{CMake + Clang}\\{\footnotesize Build System}};
    \node[comp] (libfuzzer) at (11, -2.5) {\textbf{libFuzzer}\\{\footnotesize Fuzzing Engine}};
    \node[comp] (llvmcov) at (11, -5) {\textbf{llvm-cov}\\{\footnotesize Coverage Tool}};
    \node[comp] (podman) at (5.5, -5) {\textbf{Podman}\\{\footnotesize Container Runtime}};

    % --- Data artifacts ---
    \node[data] (target) at (5.5, 2) {C++ Target Library\\{\footnotesize(yaml-cpp, etc.)}};
    \node[data] (driver) at (8, -1.25) {Generated\\Fuzz Driver};
    \node[data] (binary) at (14, -1.25) {Instrumented\\Binary};
    \node[data] (coverage) at (14, -5) {Coverage\\Report};

    % --- Arrows ---
    \draw[arrow] (ollama) -- node[above, font=\footnotesize]{OpenAI API} (cifuzz);
    \draw[arrow] (llamacpp) -- node[below, font=\footnotesize]{OpenAI API} (cifuzz);
    \draw[arrow] (target) -- (cifuzz);
    \draw[arrow] (cifuzz) -- (driver);
    \draw[arrow] (driver) -- (cmake);
    \draw[arrow] (cmake) -- (binary);
    \draw[arrow] (binary) -- (libfuzzer);
    \draw[arrow] (libfuzzer) -- (llvmcov);
    \draw[arrow] (llvmcov) -- (coverage);

    % Container boundary
    \draw[dashed, thick, gray] (3.2, -6.5) rectangle (15.5, 3);
    \node[font=\footnotesize\bfseries, gray] at (9.3, -6.8) {Podman Container};

    \draw[darrow] (podman) -- (cifuzz);
    \draw[darrow] (podman) -- (cmake);
    \draw[darrow] (podman) -- (libfuzzer);
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item Driver generation must complete within the existing CI/CD feedback window (15--30 minutes per pipeline run).
    ...
    \begin{enumerate}
        \item Annual infrastructure cost must remain below \texteuro{}2,000 for moderate usage.
...
\begin{figure}[htbp]
    ...
    \resizebox{\textwidth}{!}{
    \begin{tikzpicture}[
        step/.style={draw, rounded corners=6pt, rectangle, minimum width=3.5cm, minimum height=1cm, align=center, fill=orange!10, thick, font=\small},
    ...
    % --- Row 1 ---
    \node[step] (checkout) at (0, -2) {Checkout\\Repository};
    \node[step] (login) at (5, -2) {Login to\\JFrog Artifactory};

    % --- Row 2 ---
    \node[step] (corpus) at (5, -4) {Download\\Previous Corpus};
    \node[step] (container) at (0, -4) {Pull cifuzz\\Container Image};

    % --- Row 3 ---
    \node[step] (build) at (2.5, -6) {Build Project\\{\footnotesize(CMake + Clang)}};

    % --- Row 4: LLM ---
    \node[step, fill=yellow!20] (spark) at (2.5, -8) {\textbf{cifuzz spark}\\{\footnotesize Generate Fuzz Driver}\\{\footnotesize via Azure OpenAI}};

    % --- Row 5 ---
    \node[step] (rebuild) at (2.5, -10) {Rebuild with\\Generated Driver};

    % --- Row 6 ---
    \node[step] (fuzz) at (2.5, -12) {Run libFuzzer\\{\footnotesize(30\,s, ASan+UBSan)}};
    ...
    % --- Row 8 ---
    \node[step] (upload) at (2.5, -16) {Upload Updated\\Corpus to Artifactory};
