...
\begin{figure}[htbp]
    ...
    \node[comp] (runner) at (0, -3) {GitHub Actions\\Self-hosted Runner};

    % --- Security Boundary ---
    \node[fw] (firewall) at (0, -6.5) {Corporate\\Firewall};

    % --- Public Internet (not used) ---
    \node[internet] (internet) at (-5, -6.5) {Public\\Internet};

    % --- Azure Cloud ---
    \node[comp] (privatelink) at (0, -10) {Azure Private Link\\Endpoint};
    \node[comp] (llm) at (0, -13) {Azure OpenAI\\LLM Service};
    ...
    % --- Subgraph boundaries ---
    \draw[thick, dashed, blue!50] (-4.5, 1.2) rectangle (4.5, -4.2);
    \node[font=\footnotesize\bfseries, blue!60] at (0, 1.5) {Corporate Network (CARIAD)};

    \draw[thick, dashed, orange!50] (-3.5, -8.5) rectangle (3.5, -14.5);
    \node[font=\footnotesize\bfseries, orange!60] at (0, -8.2) {Azure Cloud};
