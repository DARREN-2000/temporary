%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Master's Thesis: AI in Automotive CI/CD/CT Pipelines
% Author: Morris Darren Babu
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{formatThesisHSCD}

% --- DOCUMENT CONFIGURATION ---
\setboolean{englishDocument}{true}
\newcommand{\studname}{Morris Darren Babu}

% --- PACKAGES ---
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{listings}
\usepackage{xcolor}
\usepackage{enumitem}
\usepackage{amsmath}
\usepackage{tikz}     
% Required for V-Model diagram and architecture diagrams
\usetikzlibrary{positioning, arrows.meta, shapes.geometric, fit, backgrounds, calc}
\usepackage[hyphens]{url}
\usepackage{eurosym}   % Proper euro symbol rendering
\usepackage{adjustbox} % Constrain diagram dimensions (max width + max height)

% --- HYPERLINK SETUP ---
\usepackage[colorlinks=true, 
    linkcolor=blue, 
    citecolor=blue, 
    urlcolor=blue,
    pdftitle={AI Usage in CI/CD/CT Pipelines},
    pdfauthor={Morris Darren Babu}
]{hyperref}

% --- BIBLIOGRAPHY SETUP ---
\usepackage[backend=biber, 
    style=numeric, 
    maxnames=80, 
    minnames=4, 
    giveninits=false, 
    defernumbers=true, 
    maxcitenames=2, 
    mincitenames=1, 
    bibencoding=utf8
]{biblatex}
\addbibresource{literature.bib} 

% --- CODE STYLING ---
\definecolor{codegray}{rgb}{0.5,0.5,0.5}
\definecolor{codeblue}{rgb}{0,0,0.6}
\definecolor{codegreen}{rgb}{0,0.5,0}

\lstdefinestyle{codestyle}{
    backgroundcolor=\color{white},   
    commentstyle=\color{codegreen},
    keywordstyle=\color{blue},
    stringstyle=\color{codeblue},
    basicstyle=\ttfamily\footnotesize,
    breakatwhitespace=false,         
    breaklines=true,                 
    captionpos=b,                    
    keepspaces=true,                 
    numbers=left,                    
    numbersep=5pt,                  
    showspaces=false,                
    showstringspaces=false,
    showtabs=false,                  
    tabsize=2,
    frame=single,
    framerule=0.1pt,
    framesep=3pt,
    rulecolor=\color{black}
}
\lstset{style=codestyle}

\begin{document}

\ifthenelse{\boolean{englishDocument}}{\selectlanguage{english}}{\selectlanguage{ngerman}}

% =========================================
% FRONT MATTER
% =========================================
\frontmatter

\codesigntitle{Master's Thesis}
{AI Usage in CI/CD/CT Pipelines for Compute Platforms in Automotive}
{\studname}
{08150815} % <<<--- REMINDER: CHECK MATRIKEL-NR
{University Supervisor: \\
Prof. Dr.-Ing. Jürgen Teich\\
Dr.-Ing. Stefan Wildermann\\[4mm]
Industrial Supervisor (CARIAD SE): \\
Dr.-Ing. Bernd Schaller\\
Dr. Andreas Weichslgartner}

\input{demoFile_aufgabe} 
\input{chapters/abstract.tex}

% --- TABLE OF CONTENTS ---
\tableofcontents

% --- LISTS ---
% This command automatically searches all chapters for \begin{figure} and lists them here
\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{List of Figures}
\listoffigures

\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{List of Tables}
\listoftables

\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{List of Abbreviations}
\chapter*{List of Abbreviations}
\markboth{List of Abbreviations}{List of Abbreviations}
\begin{itemize}
    \item[ABS] Anti-lock Braking System
    \item[ADAS] Advanced Driver-Assistance Systems
    \item[AFL] American Fuzzy Lop
    \item[AI] Artificial Intelligence
    \item[API] Application Programming Interface
    \item[ASan] AddressSanitizer
    \item[AST] Abstract Syntax Tree
    \item[AUTOSAR] AUTomotive Open System ARchitecture
    \item[BRS] Business Requirements Specification
    \item[CD] Continuous Delivery / Continuous Deployment
    \item[CI] Continuous Integration
    \item[CT] Continuous Testing
    \item[DNS] Domain Name System
    \item[ECU] Electronic Control Unit
    \item[GPU] Graphics Processing Unit
    \item[HLD] High-Level Design
    \item[JSON] JavaScript Object Notation
    \item[LLD] Low-Level Design
    \item[LLM] Large Language Model
    \item[LLVM] Low Level Virtual Machine
    \item[LoRA] Low-Rank Adaptation
    \item[MTTV] Mean Time To Vulnerability
    \item[SAE] Society of Automotive Engineers
    \item[SRS] System Requirements Specification
    \item[UBSan] UndefinedBehaviorSanitizer
    \item[UNECE] United Nations Economic Commission for Europe
    \item[XML] Extensible Markup Language
    \item[YAML] YAML Ain't Markup Language
\end{itemize}

% =========================================
% MAIN MATTER
% =========================================
\mainmatter

% Using your specific filenames
\input{chapters/introduction.tex}
\input{chapters/literature.tex}
\input{chapters/methodology.tex}
\input{chapters/implementation.tex}
\input{chapters/results.tex}
\input{chapters/discussion_conclusion.tex}

% =========================================
% BACK MATTER
% =========================================
\appendix
\chapter{Appendix}

\section{Evaluated Model Summary}
\label{sec:appendix_models}

Table~\ref{tab:appendix_all_models} provides a complete overview of all LLM models evaluated during Phase~1 of this research, including their parameter counts, quantization levels, architecture types, and primary results on yaml-cpp.

\begin{table}[htbp]
\centering
\caption{Complete List of Evaluated Models}
\label{tab:appendix_all_models}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llcccl}
\toprule
Model & Architecture & Parameters & Quantization & yaml-cpp Coverage & Status \\
\midrule
Qwen 2.5-Coder 32B & Decoder-only & 32B & Q4\_K\_M & 43.08\% & Functional \\
Gemma 3 27B & Decoder-only & 27B & Q4\_K\_M & 45.06\% & Functional \\
Phi-4 14B & Decoder-only & 14B & Q4\_K\_M & 34.26\% & Functional \\
Qwen 2.5-Coder 14B & Decoder-only & 14B & Q5\_K\_M & 0\% & Compile failure \\
Qwen 2.5-Coder 7B & Decoder-only & 7B & Q5\_K\_M & 0\% & Compile failure \\
Qwen 2.5-Coder 1.5B & Decoder-only & 1.5B & FP16 & 0\% & Compile failure \\
DeepSeek Coder 33B & Decoder-only & 33B & Q4\_K\_M & 0\% & Compile failure \\
DeepSeek Coder V2 Lite & MoE & 16B & Q4\_K\_M & 0\% & Compile failure \\
CodeLlama 34B Instruct & Decoder-only & 34B & Q4\_K\_M & 0\% & Compile failure \\
WizardCoder 34B & Decoder-only & 34B & Q4\_K\_M & 0\% & Compile failure \\
Yi 34B & Decoder-only & 34B & Q4\_K\_M & 0\% & Invalid API usage \\
Gemma 3 9B & Decoder-only & 9B & Q4\_K\_M & 0\% & Compile failure \\
Gemma 3 4B & Decoder-only & 4B & Q4\_K\_M & 0\% & Compile failure \\
Mixtral 46.7B & MoE & 46.7B & Q4\_K\_M & N/A & Hardware limit \\
\bottomrule
\end{tabular}%
}
\end{table}

\section{LoRA Fine-Tuning Script}
\label{sec:appendix_lora}

The following script shows the complete LoRA fine-tuning configuration used in Phase~2 (Section~\ref{sec:impl_phase2}).

\begin{lstlisting}[language=Python, caption={LoRA Fine-Tuning Script for Fuzz Driver Generation}, label={lst:appendix_lora}]
from transformers import AutoModelForCausalLM, AutoTokenizer, TrainingArguments
from peft import LoraConfig, get_peft_model
from datasets import load_dataset

# Model and tokenizer
MODEL_NAME = "Qwen/Qwen2.5-Coder-1.5B"
tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, torch_dtype="float16", device_map="auto")

# LoRA configuration
lora_config = LoraConfig(
    r=16,                  # Rank: controls adaptation complexity
    lora_alpha=32,         # Scaling factor
    lora_dropout=0.1,      # Dropout to prevent overfitting
    target_modules=["q_proj", "v_proj", "k_proj", "o_proj"],
    task_type="CAUSAL_LM"
)
model = get_peft_model(model, lora_config)

# Training arguments
training_args = TrainingArguments(
    output_dir="./fuzz-lora-checkpoints",
    num_train_epochs=3,
    per_device_train_batch_size=4,
    gradient_accumulation_steps=4,
    learning_rate=2e-5,
    lr_scheduler_type="cosine",
    warmup_steps=100,
    fp16=True,
    logging_steps=10,
    save_strategy="epoch",
    evaluation_strategy="epoch",
)

# Dataset format: {"input": "<headers>", "output": "<fuzz_driver>"}
dataset = load_dataset("json", data_files="ossfuzz_drivers_709.jsonl")
\end{lstlisting}

\section{Sample Generated Fuzz Driver}
\label{sec:appendix_sample_driver}

Listing~\ref{lst:appendix_driver} shows a representative fuzz driver generated by Qwen~2.5-Coder~32B for the yaml-cpp library. This driver achieved 43\% line coverage during our evaluation (Chapter~5).

\begin{lstlisting}[language=C++, caption={Sample LLM-Generated Fuzz Driver for yaml-cpp}, label={lst:appendix_driver}]
#include <cstdint>
#include <cstddef>
#include <string>
#include "yaml-cpp/yaml.h"

extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    if (size == 0) return 0;
    
    // Convert fuzzer input to string
    std::string input(reinterpret_cast<const char*>(data), size);
    
    try {
        // Parse YAML from the fuzzer-provided input
        YAML::Node doc = YAML::Load(input);
        
        // Exercise different node types
        if (doc.IsMap()) {
            for (auto it = doc.begin(); it != doc.end(); ++it) {
                std::string key = it->first.as<std::string>();
                if (it->second.IsScalar()) {
                    std::string val = it->second.as<std::string>();
                }
            }
        } else if (doc.IsSequence()) {
            for (size_t i = 0; i < doc.size(); ++i) {
                if (doc[i].IsScalar()) {
                    std::string val = doc[i].as<std::string>();
                }
            }
        }
    } catch (const YAML::Exception&) {
        // Expected for malformed input
    } catch (const std::exception&) {
        // Catch other standard exceptions
    }
    
    return 0;
}
\end{lstlisting}

\section{Pipeline Configuration}
\label{sec:appendix_pipeline}

Listing~\ref{lst:appendix_yml} shows the complete GitHub Actions workflow configuration for CI/CD/CT integration with Azure OpenAI, as discussed in Chapter~4, Section~\ref{sec:impl_phase3}. The workflow diagram in Figure~\ref{fig:cicd_workflow} visualizes this pipeline.

\begin{lstlisting}[language=yaml, caption={Complete GitHub Actions Workflow with Azure OpenAI Integration}, label={lst:appendix_yml}]
name: fuzz
env:
  PROJ_NAME: "<project-name>"
  VERSION: "0.1"
  ARTIFACTORY_CORPUS_URL: "https://<corporate-artifactory>.example.com/..."
jobs:
  fuzz:
    runs-on: [<self-hosted-runner>]
    env:
      JFROG_API_KEY: ${{ secrets.JFROG_API_KEY }}
      ARTIFACTORY_USER: ${{ secrets.ARTIFACTORY_USER }}
      PROJECT_FOLDER: "/project/"
      CIFUZZ_LLM_API_URL: "https://<endpoint>.openai.azure.com"
      CIFUZZ_LLM_AZURE_DEPLOYMENT_NAME: "gpt-4o"
      CIFUZZ_LLM_MAX_TOKENS: "40000"
      CIFUZZ_LLM_API_TOKEN: ${{ secrets.CIFUZZ_LLM_API_TOKEN }}
    steps:
      - uses: actions/checkout@v3
      - name: Login to JFrog
        run: buildah login -u $ARTIFACTORY_USER -p $JFROG_API_KEY ...
      - name: Download previous corpus
        run: |
          # Download corpus archive if it exists in Artifactory
          curl -u "$ARTIFACTORY_USER:$JFROG_API_KEY" ... | tar -xz
      - name: Pull cifuzz container
        run: |
          containerid=$(buildah from .../cifuzz:${VERSION})
          echo "containerid=$containerid" >> $GITHUB_ENV
      - name: Build project
        run: |
          buildah run -v $(pwd):/$PROJECT_FOLDER $containerid \
            /bin/bash -c "cd /project && mkdir build && cd build && cmake .. && make"
      - name: Generate fuzz driver via LLM
        run: |
          buildah run --env CIFUZZ_LLM_API_URL ... \
            /bin/bash -c "cd /project/build && cifuzz spark parser_target"
      - name: Rebuild with generated tests
        run: buildah run ... "cd /project/build && make"
      - name: Run fuzzing (60s, ASan+UBSan)
        run: |
          buildah run ... "cifuzz run parser_target \
            --libfuzzer-arg=-max_total_time=60"
      - name: Upload updated corpus
        run: |
          tar -czf ${PROJ_NAME}_corpus.gz .cifuzz-corpus
          curl -T ${PROJ_NAME}_corpus.gz ...
\end{lstlisting}

\section{Sample LLM Prompt for Fuzz Driver Generation}
\label{sec:appendix_prompt}

Listing~\ref{lst:appendix_prompt_example} shows a representative prompt sent to the LLM during fuzz driver generation. The prompt combines the extracted API context (abbreviated for space) with the fuzzing instructions that define the required driver structure. This example targets the yaml-cpp library.

\begin{lstlisting}[caption={Representative cifuzz spark Prompt (abbreviated)}, label={lst:appendix_prompt_example}]
=== SYSTEM ===
You are a security-focused C++ developer specializing in fuzz testing
with libFuzzer. Generate a fuzz driver for the target API.

=== FUZZING INSTRUCTIONS ===
1. The driver MUST implement:
   extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
2. Consume the byte buffer `data` of length `size` as input.
3. Translate bytes into valid API calls to the target library.
4. Do NOT call exit(), abort(), or _exit().
5. Do NOT use static or global state that persists across calls.
6. Catch and handle exceptions; do not let them propagate.
7. Return 0 on success.

=== EXTRACTED API CONTEXT ===
// File: yaml-cpp/yaml.h
namespace YAML {
  class Node {
  public:
    Node();
    Node Load(const std::string& input);
    bool IsMap() const;
    bool IsSequence() const;
    bool IsScalar() const;
    // ... (50+ additional methods) ...
    template<typename T> T as() const;
    iterator begin();
    iterator end();
    std::size_t size() const;
  };
  // ... (additional classes: Parser, Emitter, Exception) ...
}

=== TASK ===
Generate a complete fuzz driver that exercises the YAML::Node API.
Include all necessary #include directives.
Maximize code coverage of the target library.
\end{lstlisting}

\cleardoublepage
\phantomsection
\ifthenelse{\boolean{englishDocument}}{\renewcommand{\bibname}{References}}{\renewcommand{\bibname}{Literatur}}
\printbibliography[heading=bibintoc]

\cleardoublepage
\input{erklaerung} 

\end{document}