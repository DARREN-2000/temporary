...
The comparative performance of these models is visualized in Figure \ref{fig:model_comparison}. Gemma 3 27B and Qwen 2.5-Coder 32B achieved the highest coverage levels (45\% and 43\% respectively), while Phi-4 14B reached 34\%. General-purpose models like Yi 34B, DeepSeek R1, and Mixtral 46.7B failed to achieve any coverage.

\begin{figure}[htbp]
\centering
\caption{Code Coverage by Model (yaml-cpp)}
\label{fig:model_comparison}
\fbox{\parbox{\linewidth}{\textit{Note: This diagram should be rendered from the Mermaid chart using Mermaid Live Editor (https://mermaid.live) and inserted as a PDF or PNG image. The chart shows a bar graph comparing code coverage percentages for Gemma 3 27B (45.06\%), Qwen 2.5-Coder 32B (43.08\%), Phi-4 14B (34.26\%), Yi 34B (0\%), DeepSeek R1 (0\%), and Mixtral 46.7B (0\%). Only three of the fourteen evaluated models achieved measurable code coverage. The remaining models either produced non-functional drivers (Yi 34B, DeepSeek R1) or could not complete evaluation due to hardware constraints (Mixtral 46.7B). Additional unsuccessful models are discussed in Section~5.2.2 and Table~5.2.}}}
\end{figure}
...
The fmt library produced an unexpected result. Smaller models outperformed the larger ones that had succeeded on yaml-cpp.
