...
That said, these numbers require context. While 45\% coverage represents a notable automated achievement, analysis of comparable manually-written fuzz drivers in OSS-Fuzz projects (based on examination of yaml-cpp and similar parsing library drivers) suggests expert-crafted drivers typically achieve 60 to 80\% line coverage for libraries of similar complexity. Our best AI-generated driver falls short of expert human work in absolute coverage terms. The value proposition is not surpassing humans, it is automating what would otherwise require considerable manual effort.
...
\textbf{Cross-library summary:} The results show that model performance varies considerably across libraries. Parsing libraries (yaml-cpp, jsoncons, pugixml) consistently produced the best results, while utility libraries (glm, spdlog) were more challenging. Library complexity did not always predict difficulty, as fmt's simpler API yielded perfect coverage with smaller models. These findings suggest that API style and documentation quality matter more than library size or domain.
