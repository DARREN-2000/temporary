...
\textbf{Cross-library summary:} The results show that model performance varies substantially across libraries. Parsing libraries (yaml-cpp, jsoncons, pugixml) consistently produced the best results, while utility libraries (glm, spdlog) were more challenging. Library complexity did not always predict difficulty, as fmt's simpler API yielded perfect coverage with smaller models. These findings suggest that API style and documentation quality matter more than library size or domain.
...
One key insight is that a fine-tuned small model can match larger models while using far fewer computational resources. This makes enterprise deployment practical, as it does not require expensive GPU infrastructure to run effective fuzz driver generation.
...
\begin{itemize}
    \item Senior security engineer hourly rate: \texteuro{}80 to 120 (industry average)
...
Fourth, the economics are favorable, at least on paper. Annual infrastructure costs range from \texteuro{}74 for light usage to \texteuro{}1,452 for full enterprise deployment. Manual security engineering involves substantially higher personnel investment, though the two approaches serve complementary rather than substitutable roles, the LLM automates initial driver creation while human expertise remains necessary for review and refinement.
