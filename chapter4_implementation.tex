...
Local development happened on a MacBook Pro with an M1 Pro chip. For improved Linux container compatibility on Apple Silicon, we chose Podman as our container runtime. Podman aligned well with the enterprise CI/CD runners, which also used Linux.
...
Transitioning from Docker to Podman required adapting several scripts to account for differences in container networking behavior.
...
Our initial pipeline configuration did not account for the network isolation requirements. The Azure OpenAI requests were routed through the corporate firewall, which, as expected in a zero-trust environment, did not permit direct external API access.
...
Investigation revealed that the proxy configuration intended for Azure was inadvertently affecting internal Git operations. The runner was trying to send Git traffic through a proxy that was not configured for that purpose.
...
Each environment variable had to be passed explicitly. Careful validation of the configuration was necessary, as the pipeline did not halt on missing LLM credentials.
