...
CARIAD operates with a zero-trust security model. CI/CD runners can't access the public internet by default. Every external connection requires explicit approval. While this is appropriate for security, it creates challenges for AI integration.
...
LLM outputs are non-deterministic. Sometimes the generated driver doesn't compile. Sometimes it compiles but crashes immediately. The pipeline was designed to handle these cases gracefully.
